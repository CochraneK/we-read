...
class PrivateLabActionTests(unittest.TestCase):
    def test_optional_action_steps_cover_advisor_and_path(self):
        ...
        for expected in ('Advisor Live Candidates','Advisor Shortlist','Advisor Report','Reading Path Context','Reading Path Live Discovery','Reading Path Candidate Verification','Reading Path Candidate Info','Reading Path Final Plan','Reading Path Final Report'):
            self.assertIn(expected,labels)

    def test_final_renderer_surfaces_action_hub_and_spaced_recall(self):
        ...
        page=final.render_final(data,assets={"alchemyBook":True,"advisor":True,"pathDiscovery":True})
