...
class PublicQuotesTests(unittest.TestCase):
        ...
        (root / "weread_notes_export.json").write_text(json.dumps([
            {
                ...
                "marks": [
                    ...
                    {"chapter": "第二章", "text": "这是另一条同一本书的划线，因此不应该同时公开。", "createTime": 20},
                ],
    ...
    def test_filtered_mode_excludes_secret_and_never_publishes_reviews(self):
        with tempfile.TemporaryDirectory() as td:
            data = Path(td)
            self.make_data(data)
            result = quotes.build_public_quotes(data, include_private=False)
    ...
    def test_full_mode_can_include_secret_but_still_caps_one_per_book(self):
        with tempfile.TemporaryDirectory() as td:
            data = Path(td)
            self.make_data(data)
            result = quotes.build_public_quotes(data, include_private=True)
        self.assertEqual({x["bookId"] for x in result["items"]}, {"pub", "sec"})
        self.assertEqual(len(result["items"]), 2)
        self.assertTrue(result["policy"]["includePrivateBooks"])
    ...
    def test_augment_site_writes_explicit_public_contract(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            ...
            (site / "index.html").write_text('<html><head><style></style></head><body><nav></nav><article class="card wide privacy">privacy</article></body></html>', encoding="utf-8")
