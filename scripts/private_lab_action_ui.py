#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Add Private Lab action/report hub for Alchemy, Advisor and Reading Path."""
...
def augment(page: str, assets: dict[str, bool]) -> str:
    ...
    command = '''# 实时 Advisor 候选（需要 WEREAD_API_KEY）\npython scripts/build_private_reading_lab.py --include-private --advisor-query "主题"\n\n# Path 候选池\npython scripts/build_private_reading_lab.py --include-private --path-topic "主题"'''
    section = f'''<section class="panel" id="actions" style="margin-top:14px"><div class="section-head"><div><h2>Knowledge Actions</h2><p>从“看数据”进入“重新理解 / 找下一本 / 构建路径”。实时目录动作需要本地 <code>WEREAD_API_KEY</code>。</p></div></div><div class="action-grid">{cards}</div><div class="action-code">{html.escape(command)}</div></section>'''
