...
@media(max-width:560px){.wrap{padding-top:20px}.hero{padding-top:16px}.hero h1{font-size:42px}.hero p{font-size:14px}.hero-strip{grid-template-columns:repeat(2,1fr);gap:7px}.hero-metric{padding:10px 11px}.hero-metric b{font-size:17px}.nav{margin-inline:-2px}.nav a{padding:6px 8px;font-size:11px}.nav-control{padding:6px 8px}.chapter-heading{grid-template-columns:1fr;gap:4px;padding-top:22px}.chapter-index{font-size:10px}.chapter-heading h2{font-size:29px}.chapter-heading p{font-size:11px}.card:hover{transform:none}.archive-tools{bottom:10px}.float-btn{width:39px;height:39px}.title{align-items:flex-start;flex-direction:column;gap:3px}.title small{font-size:10px}.table{display:block;overflow-x:auto;white-space:nowrap}}
...
(()=>{
  ...
  // Hero becomes a useful at-a-glance archive cover instead of another chart.
  ...
  // Curated, sticky top-level navigation. Existing dozens of modules remain reachable within chapters.
  ...
  if(nav){
    nav.innerHTML=navItems.map(([id,label])=>`<a href="#${id}" data-chapter-link="${id}">${label}</a>`).join('')+'<span class="nav-spacer"></span><button class="nav-control" id="focusToggle" type="button" aria-pressed="false" title="隐藏次要明细，保留核心档案">精简</button>';
  }
  ...
  // Persist focus mode only in this browser; it never mutates report data.
  ...
  // Active chapter and top reading progress.
  const links=[...document.querySelectorAll('[data-chapter-link]')];
  if('IntersectionObserver' in window){
    const observer=new IntersectionObserver(entries=>{const visible=entries.filter(e=>e.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(!visible)return;links.forEach(a=>a.classList.toggle('active',a.dataset.chapterLink===visible.target.id));},{rootMargin:'-18% 0px -68% 0px',threshold:[0,.2,.6,1]});
  ...
  // Fast shelf search: slash focuses the explorer from anywhere outside form fields.
  ...
  if(typeof shelfFiltered==='function'&&typeof renderShelf==='function'&&typeof allBooks!=='undefined'&&typeof pf!=='undefined'&&$('shelfGrid')){
    ...
    const note=document.querySelector('#shelf-explorer .shelf-summary span:last-child');if(note)note.textContent='Pin 仅保存在当前浏览器，不修改微信读书；页面仍不发布原始笔记正文';
