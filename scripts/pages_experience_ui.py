...
(()=>{
  ...
  // Daily deterministic resurfacing from metadata-only recall candidates.
  ...
  if(typeof shelfFiltered==='function'&&typeof renderShelf==='function'&&typeof allBooks!=='undefined'&&typeof pf!=='undefined'&&$('shelfGrid')){
    ...
    renderShelf=function(reset=false){
      ...
      if(!rows.length){grid.innerHTML='<div class="shelf-empty">没有符合当前条件的书。<button type="button">清除筛选</button></div>';grid.querySelector('button').addEventListener('click',()=>{if(query)query.value='';if(typeof cat!=='undefined')cat.value='';pf.value='';renderShelf(true)})}
