...
def card_html(quote: dict, index: int) -> str:
    text = html.escape(str(quote.get("text") or ""))
    title = html.escape(str(quote.get("title") or "未知书名"))
    author = html.escape(str(quote.get("author") or "未知作者"))
    chapter = html.escape(str(quote.get("chapter") or ""))
    theme = html.escape(str(quote.get("theme") or "其他"))
    tier = html.escape(str(quote.get("tier") or "—"))
    score = int(quote.get("score") or 0)
    book_id = html.escape(str(quote.get("bookId") or ""), quote=True)
    search = html.escape(" ".join([text, title, author, chapter, theme]).lower(), quote=True)
    deep_link = f"weread://reading?bId={book_id}" if book_id else ""
    open_book = f'<a class="book-link" href="{deep_link}">打开微信读书</a>' if deep_link else ""
    return f'''<article class="quote-card" tabindex="0" data-search="{search}" data-theme-label="{theme}" aria-label="第 {index} 张划线卡片，点击翻转">
...
<h1>我的微信读书 · 划线卡片</h1>
<p class="sub">从个人划线中筛选、去重并按启发式“金句度”排序。划线是保存的原文，不自动代表你的观点；自己的想法应以 review 为证据。</p>
...
def main():
    ...
    page = render(payload, include_all=args.all)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(page, encoding="utf-8")
    print(f"quote-cards: {args.output} | cards={len(selected_quotes(payload, args.all))} private=true")
